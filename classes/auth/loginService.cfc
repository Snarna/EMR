...
<cffunction name="doLogin" access="remote" output="false" returntype="boolean">
		...
		<cfoutput>soup</cfoutput>
		<!--- To determine if user is logged in --->
		<cfset var isLoggedIn = false />
		<!--- Get data from db --->
		<cfquery name="rsLoginUser" datasource="capstone" result="loginResults">
		SELECT Providers.proFname, Providers.proLname, Providers.providerId, Providers.proEmail, Providers.proPassword FROM Providers WHERE Providers.proEmail = '#arguments.proEmail#' AND Providers.proPassword = '#arguments.proPassword#'
		</cfquery>
